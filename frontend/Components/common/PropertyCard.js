...
function PropertyCard({ property, i, isSelerPage, handleDelete, handleShowInterested }) {
    ...
    const handleLike = async () => {
        try {
            ...
            if (typeof window !== 'undefined') {
                if (liked) {
                    await axios.post(`${process.env.BASE_URL}/api/properties/${property._id}/unlike`, {}, {
                        headers: {
                            Authorization: `Bearer ${localStorage.getItem('token')}`
                        }
                    });
                    ...
                    await axios.post(`${process.env.BASE_URL}/api/properties/${property._id}/like`, {}, {
                        headers: {
                            Authorization: `Bearer ${localStorage.getItem('token')}`
                        }
                    });
    ...
    const handleInterested = () => {
        axios.post(`${process.env.BASE_URL}/api/properties/${property._id}/interested`, {}, {
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`
            }
        })
    ...
    return (
        <div key={i} className='propertyCard'>
            <Link className='propertyTextCard'  href={`/property/${property._id}`}>
                <div>
